@foreach($trip->user as $user)
    <form class="form-horizontal" role="form" method="POST" action="{{ route('store_hours', $user->pivot->id) }}">
        ...
        <tr style="color: blue;">
            ...
            <td><input id="unit" type="text" class="form-control" name="unit{!! $user->pivot->id !!}" value="{!! $user->pivot->unit !!}" required autofocus></td>
            ...
            <td><input id="bank" type="text" class="form-control" name="bank{!! $user->pivot->id !!}" value="{!! $user->pivot->bank !!}" required autofocus></td>
